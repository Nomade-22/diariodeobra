...
export async function GET(_req: Request, ctx: Ctx) {
  ...
  const registro = found[0];
  if (!registro) {
    return NextResponse.json({ error: "Registro not found" }, { status: 404 });
  }

  const funcionarios = rows<RegistroFuncionario>(
    await sql`
...
export async function PUT(req: Request, ctx: Ctx) {
  ...
  await sql`
    UPDATE registros SET
      ...
      foto_inicio_key = ${foto_inicio_key},
      foto_fim_key = ${foto_fim_key},
      foto_observacoes_key = ${foto_observacoes_key},
...
export async function DELETE(_req: Request, ctx: Ctx) {
  ...
  // Drop the funcionario links first, then the registro itself. Photo rows only
  // hold URLs — the platform owns the uploaded objects, so nothing is deleted
  // from storage here.
  await sql`DELETE FROM registro_funcionarios WHERE registro_id = ${registroId}`;
